...
### 2.1 基本カラー
- **メインカラー**: #1A1A1A
  - 用途：ヘッダー、フッター、主要な構造要素の背景
  - 変数名：var(--main-color)

- **アクセントカラー**: #483D8B
  - 用途：強調、ボタン、リンクのホバー状態
  - 変数名：var(--accent-color)

- **背景カラー**: #F5F5F5
  - 用途：ページ背景、カード背景
  - 変数名：var(--background-color)
...
- **反転文字カラー**: #FFFFFF
  - 用途：暗い背景上のテキスト（ヘッダー、フッター内）
  - 変数名：var(--inverted-text-color)

### 2.2 カラー使用ガイドライン
- すべての色指定はグローバルCSS変数を使用
- 直接的なカラーコードの使用は禁止（SNSアイコン等の例外を除く）
- メインカラーの背景上のテキストには必ず反転文字カラーを使用
- アクセントカラーは控えめに、重要な要素の強調にのみ使用
...
### 6.2 ボタン設計
- **プライマリ**: ダークグレー背景(#1A1A1A)、白文字
- **セカンダリ**: 白背景、ディープパープル文字(#483D8B)、ディープパープルボーダー
